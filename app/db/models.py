...
class Direction(db.Model):
    ...
    duration = db.Column(db.String(50), nullable=True)            # Длительность курса
    target_audience = db.Column(db.String(100), nullable=True)    # Целевая аудитория
...
class Subject(db.Model):
    ...
    status = db.Column(db.String(50), nullable=False, default='active')                 # Статус предмета (активен или нет)
...
class Olympiad(db.Model):
    ...
    status = db.Column(db.String(50), nullable=False, default='upcoming')            # Статус олимпиады (будущая/прошедшая)
...
class OlympiadRegistration(db.Model):
    ...
    status = db.Column(db.String(50), nullable=False, default='pending')              # Статус регистрации (ожидает/подтверждена)
...
class Result(db.Model):
    ...
    status = db.Column(db.String(50), nullable=False, default='not_checked')   # Статус результата (не проверен/проверен)
